function AddEllipseAtomicArray(rad, X0, Y0, VX0, VY0, InitDist, Temp, Type)
global C
...
a = rad * AtomSpacing * 1.5;  % horizontal radius
b = rad * AtomSpacing * 0.8;  % vertical radius

numAtoms = 0;
for i = 1:2*rad
    for j = 1:2*rad
        if (xp(i)^2)/a^2 + (yp(j)^2)/b^2 <= 1
            numAtoms = numAtoms + 1;
